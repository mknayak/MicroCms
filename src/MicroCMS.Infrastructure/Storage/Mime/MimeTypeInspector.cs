...
/// Falls back to the extension map from <see cref="MediaOptions.ExtensionMimeTypes"/> when
...
public sealed class MimeTypeInspector : IMimeTypeInspector
{
    ...
    /// <summary>Parameterless constructor used by tests; applies default <see cref="MediaOptions"/> values.</summary>
    public MimeTypeInspector() : this(Options.Create(new MediaOptions())) { }

    public MimeTypeInspector(IOptions<MediaOptions> options)
    {
        _extensionMap = new Dictionary<string, string>(
            options.Value.ExtensionMimeTypes,
...
?? DetectAudioVideoMime(bytes, length);
   ...
   && bytes[6] == 0x79 && bytes[7] == 0x70)
    ...
    private string? GetMimeFromExtension(string fileName)
    {
        var ext = Path.GetExtension(fileName);
        return _extensionMap.TryGetValue(ext, out var mime) ? mime : null;
    }
}
