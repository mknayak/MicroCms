...
/// <summary>Handles <see cref="SchedulePublishCommand"/>.</summary>
public sealed class SchedulePublishCommandHandler(
    IRepository<Entry, EntryId> entryRepository)
    : IRequestHandler<SchedulePublishCommand, Result<EntryDto>>
{
    public async Task<Result<EntryDto>> Handle(
        SchedulePublishCommand request,
        CancellationToken cancellationToken)
    {
        ...
        entry.SchedulePublish(request.PublishAt, request.UnpublishAt);
