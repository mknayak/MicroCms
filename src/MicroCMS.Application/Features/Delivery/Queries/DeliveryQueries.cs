...
// ── Delivery queries are authenticated at the HTTP layer via X-Api-Key.
// The MediatR AuthorizationBehavior is bypassed with [AllowAnonymousRequest]
// on every query — the controller's [Authorize(Policy = DeliveryPolicies.ApiKeyAuthenticated)]
// already enforces auth before MediatR is ever called.

/// <summary>Returns a single published entry by slug, site, and locale.</summary>
